...
class RegistrationSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if (
            municipality
            and municipality.district_id != district.id
        ):
            raise serializers.ValidationError({
                'municipality_id':
    ...
    def _create_user(
        ...
        municipality = validated_data.pop(
            'municipality'
        )
...
class StudentRegistrationSerializer(
    ...
    def create(self, validated_data):
        student_id_card = validated_data.pop(
            'student_id_card',
            None
        )
...
class InstructorRegistrationSerializer(
    RegistrationSerializer
):
    ...
    def create(self, validated_data):
        ...
        subject_expertise = validated_data.pop(
            'subject_expertise'
        )
...
class ForgotPasswordSerializer(
    ...
    def save(self):
        ...
        send_mail(
            ...
            message=(
                f'Your SkillSikka password reset OTP '
                f'is {otp}. '
...
class VerifyPasswordResetOTPSerializer(
    serializers.Serializer
):
    email = serializers.EmailField()
...
class LessonSerializer(
    ...
    def validate(self, attrs):
        ...
        if course.course_type == 'academic':
            if (
                course.subject is None
                or course.grade is None
            ):
                raise serializers.ValidationError({
                    'course': (
                        'An academic course must have '
                        'a subject and grade.'
                    )
                })
            ...
            if topic is not None:
                raise serializers.ValidationError({
                    'topic': (
                        'Skill course lessons cannot be '
                        'linked to an academic topic.'
                    )
                })
...
class EnrollCourseSerializer(
    ...
    def validate(self, attrs):
        ...
        if course.course_type != 'skill':
            raise serializers.ValidationError({
                'detail': (
                    'Only skill development courses '
                    'require enrollment.'
                )
            })
...
class QuestionOptionManagementSerializer(
    serializers.ModelSerializer
):
    class Meta:
...
class QuestionManagementSerializer(
    serializers.ModelSerializer
):
...
class QuizManagementSerializer(
    serializers.ModelSerializer
):
